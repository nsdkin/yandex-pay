...
describe('Acceptance. Скриншотные тесты', () => {
    ...
    afterEach(async function () {
        const bro = this.browser;
        await bro.yaLogoutUser(TestData.UserCard1WithoutAva);
    });
    it('yandexpay-121: Перс. кнопка. У пользователя есть карта, нет аватарки', async function () {
        const bro = this.browser;
        await bro.pause(1000); //для отработки анимации отрисовки перс кнопки поверх обычной
        await bro.yaAssertView('user-with-card-without-ava-121', demo.pay_button());
    });
});
