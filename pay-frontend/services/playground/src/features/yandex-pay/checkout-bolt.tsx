...
import { getCheckoutSheetBolt, getDynamicCartOption } from 'store/selectors';
...
export const YandexPayCheckoutBold: React.FC<YandexPayProps> = function YandexPayCheckoutBold({
    className,
    onReady = noop,
    onProcess = noop,
    onSuccess = noop,
    ...
    const setup = useCallback(async () => {
        ...
        const checkout = await YaPay.createCheckout(checkoutSheet);

        if (unmountRef.current) {
            return checkout.destroy();
        }

        checkout.mountButton(buttonContainerRef.current, {
        ...
        checkout.on(Sdk.PaymentEventType.Ready, (event) => {
            log({
        ...
        checkout.on(Sdk.PaymentEventType.Success, (event) => {
            log({
                sender: LogRecordOwner.Sdk_Payment,
                receiver: LogRecordOwner.PG_React,
                message: event,
        ...
        checkout.on(Sdk.PaymentEventType.Abort, (event) => {
            log({
                sender: LogRecordOwner.Sdk_Payment,
                receiver: LogRecordOwner.PG_React,
                message: event,
        ...
        checkout.on(Sdk.PaymentEventType.Error, (event) => {
            log({
                sender: LogRecordOwner.Sdk_Payment,
                receiver: LogRecordOwner.PG_React,
                message: event,
    ...
    }, [checkoutSheet, onSuccess, onAbort, onError, onReady, onInvalidMerchant, log]);
    ...
    useEffectOnce(() => {
        if (hasDynamicCart) {
            setTimeout(() => {
                ...
                const { metadata } = checkoutSheet;
                const cart = clone(checkoutSheet.cart);

                cart.items.push(...cart.items);

                checkoutRef.current?.update({ cart, metadata });
